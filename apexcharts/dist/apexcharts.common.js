...
var Series =
  ...
  function () {
    function Series (ctx) {
      ...
      if (ctx.w.config.series.data !== undefined) {
        // console.log('append data is ' + ctx.w.config.series.data);

        // console.log('0 is ' + ctx.w.config.series[0].data);
        ctx.w.config.series[0].data = ctx.w.config.series.data;
        // console.log('refreshed 0 is ' + ctx.w.config.series[0].data);
        var index = ctx.w.config.series.indexOf('data')
        if (index > -1) {
          ctx.w.config.series.splice(index, 1);
        }
      }
      this.w = ctx.w;
    }
